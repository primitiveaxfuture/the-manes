...
**ルートの `LICENSE` は、この版で現に公開されている物のライセンスを置く。** v1.0.0 で本文が公開されたため、`LICENSE` は本文の条件（CC BY-ND 4.0）である。用語集・命題一覧・機械可読スキーマ・付属文書は従前どおり CC BY 4.0——先行公開（v0.1.0）で CC BY 4.0 の下に公開された物の条件が、事後に狭まることはない。
